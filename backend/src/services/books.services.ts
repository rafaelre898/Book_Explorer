...
import { DEFAULT_RATING, GOOGLE_BOOKS_API_URL, UNKNOWN } from "../utils/constants";
...
async function getBooks(limit: number, offset: number, search: string) {
    ...
    return books.map((book: any) => ({
        ...
        publication_year: book.volumeInfo.publishedDate ? book.volumeInfo.publishedDate.split("-")[0] : UNKNOWN,
